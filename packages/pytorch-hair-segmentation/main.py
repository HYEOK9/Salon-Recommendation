...
if __name__ == "__main__":
    ...
    # prepare images
    img_paths = [
        os.path.join(img_dir, k) for k in sorted(os.listdir(img_dir)) if has_img_ext(k)
    ]
    with torch.no_grad():
        for i, img_path in enumerate(img_paths, 1):
            ...
            path = os.path.join(save_dir, "result-" + os.path.basename(img_path))
            ...
            cv2.imwrite(path, image_n_cropped)
